...
def df_shrink_dtypes(df, skip=[], obj2cat=True, int2uint=False):
    ...
    for c, old_t in filter(exclude, df.dtypes.items()):
        # MODIFIED
        if df[c].apply(lambda x: isinstance(x, (torch.Tensor, np.ndarray))).any():
            continue
...
class Tabular(CollBase, GetAttr, FilteredBase):
    ...
    def __init__(self, df, procs=None, cat_names=None, cont_names=None, vec_names=None, y_names=None, y_block=None, splits=None,
        ...
        self.cat_names,self.cont_names,self.vec_names,self.procs = L(cat_names),L(cont_names),L(vec_names),Pipeline(procs)
    ...
    def new(self, df, inplace=False):
        return type(self)(df, do_setup=False, reduce_memory=False, y_block=TransformBlock(), inplace=inplace,
                          **attrdict(self, 'procs','cat_names','cont_names','vec_names','y_names', 'device'))
    ...
    def x_names (self): return self.cat_names + self.cont_names + self.vec_names
...
class ReadTabBatch(ItemTransform):
    ...
    def encodes(self, to):
        if not to.with_cont: res = (tensor(to.cats).long(),)
            ...
            if to.vec_names:
                all_cat_vecs = []
                for i in range(len(to.vec_names)):
                    vec_list = []
                    for _, row in to.vecs.iterrows():
                        vec_tensor = torch.as_tensor(row[to.vec_names[i]], dtype=torch.float32)
                        vec_list.append(vec_tensor)
                    full_cat_vec_tensor = torch.stack(vec_list)
                    all_cat_vecs.append(full_cat_vec_tensor)
                res = (tensor(to.cats).long(),tensor(to.conts).float(), all_cat_vecs)
            else:
