...
def tabular_learner(
    ...
    # MODIFIED
    if dls.vec_names: vec_sizes = [to.vecs[col].iloc[0].shape[0] for col in dls.vec_names]
    else: vec_sizes = []
    model = TabularModel(emb_szs, len(dls.cont_names), n_out, layers, y_range=y_range, vec_sizes=vec_sizes, **config)
